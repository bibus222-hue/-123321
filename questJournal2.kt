//1.1 300. Ответ: 300
//1.2 для рассылки события. Ответ: A
//1.3 всего написано ровно 14352. Ответ: C
...
Column {
    QuestProgressBar(questJournal)

    LazyColumn {
        items(questJournal.quests) { quest ->
            QuestItem(
                quest = quest,
                onStartClick = { questJournal.startQuest(quest.id) },
                onCompleteClick = { questJournal.completeQuest(quest.id) }
            )
        }
    }
}
...
fun QuestItem(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = "Статус: ${quest.status.name}",
                style = MaterialTheme.typography.bodySmall
            )

            if (quest.unlockRequiredQuestId != null) {
                Text(
                    text = "Требуется квест: $quest.unlockRequiredQuestId",
                    style = MaterialTheme.typography.bodySmall
                )
            }
